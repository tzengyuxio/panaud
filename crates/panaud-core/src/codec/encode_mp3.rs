...
pub fn encode_mp3(audio: &AudioData, path: &Path) -> Result<()> {
    ...
    builder
        ...
        .map_err(|e| {
            PanaudError::encode(
                path,
                format!("set channels: {e}"),
                "check audio parameters (sample rate, channels)",
            )
        })?;
    ...
    let encoded_size = encoder.encode(input, &mut mp3_buf).map_err(|e| {
        PanaudError::encode(
            path,
            format!("MP3 encoding failed: {e}"),
    ...
    std::fs::write(path, mp3_data).map_err(|e| {
        PanaudError::encode(
            path,
            e.to_string(),
