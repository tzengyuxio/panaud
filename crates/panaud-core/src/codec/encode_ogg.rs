...
pub fn encode_ogg(audio: &AudioData, path: &Path) -> Result<()> {
    ...
    encoder.encode_audio_block(&channel_refs).map_err(|e| {
        PanaudError::encode(
            path,
            format!("OGG encoding failed: {e}"),
